...
def run_analysis(document_id: uuid.UUID, db: Session) -> None:
    ...
    try:
        ...
        for analyzer in applicable:
            current_step = f"analyzer:{analyzer.name}"
            ...
            if result.status == "success":
                any_succeeded = True
                current_step = f"merge:{analyzer.name}"
        ...
        tb_summary = traceback.format_exc()[:800]
        try:
            db.rollback()
            db.add(AuditEvent(
                document_id=document_id,
                event_type=f"pipeline_failed:{current_step}",
                event_data={"error": str(exc)[:200], "traceback": tb_summary},
            ))
            doc_obj = db.get(Document, document_id)
            if doc_obj:
